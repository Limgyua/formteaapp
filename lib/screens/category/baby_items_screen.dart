...
class _BabyScreenState extends State<BabyScreen> {
  List<Map<String, dynamic>> babyItems = [];
  bool isLoading = true;
  ...
  Future<void> _loadBabyItems() async {
    setState(() {
      isLoading = true;
    });
    final items = await DBHelper.getUsedItemsByCategory('유아용품');
    setState(() {
      babyItems = items;
      isLoading = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : babyItems.isEmpty
              ? const Center(
                  child: Text('아직 등록된 유아용품이 없습니다.',
                      style: TextStyle(color: Colors.grey, fontSize: 16)))
              : ListView.separated(
                  padding: const EdgeInsets.all(20),
                  itemCount: babyItems.length,
                  separatorBuilder: (_, __) => const SizedBox(height: 16),
                  itemBuilder: (context, idx) {
                    final item = babyItems[idx];
                    String priceText = '';
